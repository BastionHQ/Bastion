    ...
    <div wire:init="load">
        @if($readyToLoad)
            @if(!empty($keys))
                    ...
                    @foreach($keys as $key)
                        <li class="Box-row border-bottom">
                            <div class="d-flex flex-items-center">
                                <div class="Box-row text-center d-flex flex-column border-0">
                                    <span>
                                        <svg height="32" aria-hidden="true" viewBox="0 0 24 24" version="1.1" width="32"
                                             data-view-component="true"
                                             class="octicon octicon-key">
                                            <path d="M16.75 8.5a1.25 1.25 0 100-2.5 1.25 1.25 0 000 2.5z"></path><path
                                                fill-rule="evenodd"
                                  ...
                                  <span class="border d-block f6 mt-1 px-1 rounded color-text-secondary">SSH</span>
                                </div>
                                <div class="flex-auto">
                                    <strong class="d-block">{{ $key->name }}</strong>
                                    <span title="Отпечаток публичного ключа">
                                        <code style="overflow-wrap: anywhere">{{ $key->fingerprint }}</code>
                                    </span>
                                    <span class="color-text-secondary d-block">Добавлен {{ $key->created_at }}</span>
                                </div>
                                <div>
                                    <summary class="btn btn-sm btn-danger ml-1" role="button">
                                        Удалить
                                    </summary>
                                </div>
                            </div>
                ...
                <livewire:key-create-button :user="$user"/>
            @else
                <div class="Box">
                    <div class="blankslate">
                        <h3 class="mb-1">Нет ключей</h3>
                        <p>У пользователя нет ни одного ssh-ключа</p>
